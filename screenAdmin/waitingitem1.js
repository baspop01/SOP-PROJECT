...
const waitingitem1 = () => {
  ...
  const processingOrder = (id) => {
    axios.get("http://localhost:8082/order-service/order/processing/"+id)
  ...
  const deleteOrder = (row) => {
    ...
    let array = row.food.replace("_(", "@").replace(")", "@").split("@");
  ...
  useEffect(() => {
    ...
    return (
        <View style={styles.container} className="body">
            ...
            <div className="div3">
                <div style={{ display: 'flex', justifyContent: 'center', marginRight: '2vw' }}>
                    <Link to="/waiting1"><button className="btn-Click" variant="contained" style={{ cursor: 'pointer' }}>
